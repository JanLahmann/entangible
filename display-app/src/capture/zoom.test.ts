...
describe('cropRect (streamed JPEG region)', () => {
  ...
  it('rounds to integer pixels and centers at 3×', () => {
    expect(cropRect(3, 1920, 1080)).toEqual({ sx: 640, sy: 360, sw: 640, sh: 360 });
  });
  it('stays in-bounds for odd dimensions', () => {
...
describe('pinchZoom', () => {
  ...
  it('clamps to the active range', () => {
    expect(pinchZoom(2, 100, 400)).toBe(ZOOM_MAX);
