 ...
 * (0-7, one per clockwise 45° step). The mapping is the natural one — THE ANGLE
 * IS THE PHYSICAL TURN, wrapped to (−π, π]: a quarter circle clockwise gives
 * π/2, the same turn the other way gives −π/2. r = 0 is the identity 0.0 and is
 * still EMITTED as a gate rather than dropped, so a dial lying on the board is
 * always visible on screen. Distinct from `ROTATION_ANGLES`, the fixed-angle
 * print set of the classic rotation tiles. Mirrors `markers.DIAL_ANGLES`.
 */
export const DIAL_ANGLES = [
  0,
  Math.PI / 4,
  Math.PI / 2,
  (3 * Math.PI) / 4,
...
export interface GateSpec {
   ...
   * tile's board-frame rotation r (0-7, clockwise 45° steps) as `DIAL_ANGLES[r]`
   * and emitted like a classic rotation tile — including at r = 0, where the
   * emitted angle is 0. Undefined for every non-dial tile.
   */
...
function buildMarkerTable(): Map<number, GateSpec> {
  ...
  // rotation (DIAL_ANGLES[r], eight 45° steps); `parameter` stays unset,
  // `dialAxis` names it.
 ...
 * tile's rotation resolves to the same index in both detectors. The coarse
 * image-frame turn; dials read the finer `octantRotation`.
 */
...
/**
 * Clockwise 45° step index (0-7) of a marker's printed top-left corner offset
 * `(dx, dy)` (dx right, dy down) from the marker centre — the finer twin of
 * `quadrantRotation` and the value a DIAL tile reads. At canonical orientation
 * that corner sits top-left of centre (dx<0, dy<0) → 0; each clockwise 45° turn
 * advances by one: 0 = TL corner, 1 = top edge, 2 = TR corner, 3 = right edge,
 * 4 = BR corner, 5 = bottom edge, 6 = BL corner, 7 = left edge. Even r agrees
 * with `quadrantRotation` as `r / 2`. Byte-for-byte identical to
 * `markers.octant_rotation` in Python, so a dial's angle (`DIAL_ANGLES[r]`)
 * resolves the same in both detectors.
 */
export function octantRotation(dx: number, dy: number): number {
  const angle = Math.atan2(dy, dx);
  return ((Math.round((angle + (3 * Math.PI) / 4) / (Math.PI / 4)) % 8) + 8) % 8;
}
