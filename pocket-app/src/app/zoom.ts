/**
 * Pure camera-zoom math + persistence for Entangible Pocket.
 *
 ...
 *    `track.applyConstraints({ advanced: [{ zoom }] })`. The sensor does the
 *    work; we never crop. The active range comes from the capability.
 *  - **digital** — no native zoom, so we center-crop `1/zoom` of the frame in
 *    the existing `drawImage` (source-rect form) and process that region. This
 *    raises pixels-per-marker because the frame is grabbed at 1080p. The range
 *    is the fixed 1.0–3.0 / step 0.25 below.
 */
